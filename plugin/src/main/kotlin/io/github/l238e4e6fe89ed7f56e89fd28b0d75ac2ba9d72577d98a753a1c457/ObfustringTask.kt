...
abstract class ObfustringTask : DefaultTask() {
    ...
    private fun obfustringEncodeSourceFile(file: File, encoder: ObfStr) {
        ...
        val indexesOfClass = listOfFileLines.mapIndexed { index, line ->
            if (line.contains(Templates.obfustringThisAnnotation)) index else -1
        }.filter { it != -1 }
    ...
    private fun encodeValue(
        ...
        val newValue = Templates.encodedValue(encoder, stringToEncode)
    ...
    private fun checkLineForCompatibility(
        ...
        return line.contains(Templates.alreadyEncodedString) || trimmedLine.startsWith("const val") || (trimmedLine.startsWith(
            '\"'
    ...
    private fun checkLineForLogExisting(
        ...
        return if (trimmedLine.startsWith("Log.") && !trimmedLine.startsWith(Templates.alreadyEncodedLog)) {
            ...
            return {
                val lineToPaste = Templates.encodedLog(stringBuilder)
                listOfFileLines.run {
                    if (!get(indexOfCurrentLine - 1).contains(Templates.alreadyEncodedLog) && none { it == lineToPaste }) {
